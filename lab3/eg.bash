...
echo "Today is $(date)"
soon=$(date -d "now +3 hours")
day=$( echo $soon | cut -d' ' -f1 )
echo "In 3 hours the day will be $day"

# Here Documents
read -p "Do you want email? (y/[n]) => " resp
if [[ $resp =~ [Yy] ]] ; then
	mail -s"Greetings from $(uname -n)" -- $user@drexel.edu <<-EOS
	...
	To code with the best of the dorks.
